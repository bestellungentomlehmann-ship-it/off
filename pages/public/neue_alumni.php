<?php
/**
 * Neue Alumni – public registration form
 * Access: public (no authentication required)
 *
 * Form submission is handled client-side via fetch → api/public/submit_neue_alumni.php
 *
 * Security hardening:
...
$title = 'Neue Alumni Registrierung – IBC Intranet';

ob_start();
?>

<!-- ============================================================
     Neue Alumni – Registration Form
     ============================================================ -->
<style>
...
.alumni-radio-label:hover {
    border-color: rgba(255, 255, 255, 0.25);
    background: rgba(255, 255, 255, 0.09);
    color: rgba(255, 255, 255, 0.9);
}
.alumni-radio-label input[type="radio"] {
    accent-color: var(--ibc-green);
    width: 16px;
    height: 16px;
    flex-shrink: 0;
}
.alumni-radio-label:has(input:checked) {
    border-color: rgba(0, 166, 81, 0.6);
    background: rgba(0, 166, 81, 0.08);
    color: #ffffff;
    box-shadow: 0 0 0 2px rgba(0, 166, 81, 0.14);
}
...
<div class="alumni-page-wrap">
    <div class="alumni-card" role="main">
        ...
        <div class="alumni-heading">
            <h1>Neue Alumni Registrierung</h1>
            <div class="alumni-divider" aria-hidden="true"></div>
            <p>Werde Teil des IBC Alumni-Netzwerks.<br>Füll das Formular aus – wir kümmern uns um deinen Zugang.</p>
        </div>
        ...
        >
            ...
            <p>
                Wir haben deine Registrierungsanfrage erhalten und melden uns so bald wie möglich
        ...
        >
            ...
            >
                ...
                <div class="alumni-field">
                    <label for="new_email">
                        E-Mail-Adresse <span class="req" aria-label="Pflichtfeld">*</span>
                    </label>
                    <input
                        type="email"
                        id="new_email"
                    ...
                    <span id="new_email_hint" class="alumni-hint">
                        Diese Adresse erhält deinen Alumni-Zugang.
                    </span>
                </div>
                ...
                <div class="alumni-field">
                    ...
                    <input
                        ...
                        autocomplete="email"
                        inputmode="email"
                        placeholder="vorname.nachname@ibc-students.de"
                        maxlength="254"
            ...
            <!-- ── STEP 2: Studium + Alumni Vertrag + reCAPTCHA + Absenden ── -->
            <div
                id="alumniStep2"
                class="alumni-step"
            ...
            >
                ...
                <!-- Alumni Vertrag question -->
                <div class="alumni-field">
                    <label>
                        Hast du bereits einen Alumni Vertrag erhalten?
                        <span class="req" aria-label="Pflichtfeld">*</span>
                    </label>
                    <div class="alumni-radio-group" role="radiogroup" aria-label="Alumni Vertrag erhalten?">
                        <label class="alumni-radio-label">
                            <input
                                type="radio"
                                name="has_alumni_contract"
                                id="contract_yes"
                                value="1"
                                required
                                aria-required="true"
                    ...
                    <span class="alumni-hint">
                        Falls nein, erhältst du den Alumni Vertrag zusammen mit deinen Zugangsdaten per E-Mail.
                    </span>
                </div>

                <?php if (RECAPTCHA_SITE_KEY !== ''): ?>
                <div
                    id="recaptcha-neue-alumni"
                    data-theme="dark"
                ></div>
                <?php endif; ?>
...
<script>
/* ================================================================
   Neue Alumni – Wizard Form Handler (2-Schritt-Prozess)
   - Strict CSP-friendly (no eval, no inline handlers)
   - Step 1: Name + E-Mail → Step 2: Studium + Alumni Vertrag + reCAPTCHA + Submit
   ================================================================ */
(function () {
    'use strict';
    ...
    var form      = document.getElementById('alumniRegistrationForm');
    ...
    function validateStep1() {
        ...
        if (!ne)                  return 'Bitte eine E-Mail-Adresse eingeben.';
        if (!EMAIL_RE.test(ne))   return 'Die E-Mail-Adresse hat ein ungültiges Format.';
    ...
    function validateStep2() {
        ...
        var contractSelected = document.querySelector('input[name="has_alumni_contract"]:checked');

        if (!gs) return 'Bitte Abschlusssemester eingeben.';
        if (!sp) return 'Bitte Studiengang eingeben.';
        if (!contractSelected) return 'Bitte angeben, ob du bereits einen Alumni Vertrag erhalten hast.';
        return null;
    }
                    ...
                    if (typeof grecaptcha !== 'undefined' && typeof grecaptcha.ready === 'function') {
                        grecaptcha.ready(function () {
                            if (recaptchaWidgetId === null) {
                                recaptchaWidgetId = grecaptcha.render('recaptcha-neue-alumni', {
                                    sitekey: SITE_KEY,
                                    theme:   'dark'
                                });
                            }
    ...
    form.addEventListener('submit', function (e) {
        ...
        var contractEl = document.querySelector('input[name="has_alumni_contract"]:checked');
        ...
        var payload = {
            ...
            has_alumni_contract:  contractEl ? contractEl.value : '0',
        };

        fetch('/api/public/submit_neue_alumni.php', {
            method:  'POST',
            headers: {
