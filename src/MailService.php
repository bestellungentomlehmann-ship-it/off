...
<body style="margin: 0; padding: 0; background-color: #f5f5f7;">
    ...
    public static function sendHelperConfirmation($toEmail, $toName, $event, $slot, $icsContent, $googleCalendarLink) {
        ...
        $subject = "Danke für deine Anmeldung als Helfer bei " . $event['title'];
    ...
    private static function buildHelperConfirmationBody($userName, $event, $slot, $googleCalendarLink) {
        ...
        <p class="email-text">Danke für deine Anmeldung als Helfer bei ' . htmlspecialchars($event['title']) . '! Hier sind die Details zu deinem Einsatz:</p>
    ...
    /**
     * Send shop order confirmation email after successful payment
     *
     * @param string $toEmail    Recipient email address
     * @param string $toName     Recipient full name
     * @param array  $order      Order data (keys: id, total_amount, payment_method, created_at)
     * @param array  $items      Array of order items (each with: product_name, variant_value, quantity, price_at_purchase)
     * @return bool Success status
     */
    public static function sendShopOrderConfirmation(string $toEmail, string $toName, array $order, array $items): bool {
        if (self::isVendorMissing()) {
            error_log("Cannot send shop order confirmation: Composer vendor missing");
            return false;
        }

        $subject = "Bestellbestätigung #" . $order['id'];

        $bodyContent = '<p class="email-text">Hallo ' . htmlspecialchars($toName) . ',</p>
        <p class="email-text">vielen Dank für deine Bestellung! Deine Zahlung ist eingegangen und deine Bestellung wird bearbeitet.</p>
        <table class="info-table">';

        foreach ($items as $item) {
            $productName = htmlspecialchars($item['product_name'] ?? '');
            if (!empty($item['variant_value'])) {
                $productName .= ' (' . htmlspecialchars($item['variant_value']) . ')';
            }
            $quantity = (int) ($item['quantity'] ?? 1);
            $lineTotal = number_format((float) ($item['price_at_purchase'] ?? 0) * $quantity, 2, ',', '.');
            $bodyContent .= '<tr>
                <td>' . $productName . ' &times; ' . $quantity . '</td>
                <td>' . $lineTotal . ' €</td>
            </tr>';
        }

        $total = number_format((float) ($order['total_amount'] ?? 0), 2, ',', '.');
        $bodyContent .= '<tr style="font-weight:bold;">
                <td>Gesamtbetrag</td>
                <td>' . $total . ' €</td>
            </tr>
        </table>
        <p class="email-text">Bei Fragen zu deiner Bestellung kannst du uns jederzeit kontaktieren.</p>
        <p class="email-text">Viele Grüße,<br>dein IBC-Team</p>';

        $htmlBody = self::getTemplate('Bestellbestätigung', $bodyContent);
